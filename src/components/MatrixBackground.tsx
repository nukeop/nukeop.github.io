...
import { MATRIX_FRAGMENT_SHADER, VERTEX_SHADER } from "../lib/shaders";
import { cleanupGlResources, setupFullscreenQuad } from "../lib/webgl";

export const MatrixBackground: FC<{ className?: string }> = ({
	className = "",
}) => {
	...
	useEffect(() => {
		...
		const res = setupFullscreenQuad(gl, VERTEX_SHADER, MATRIX_FRAGMENT_SHADER);
		if (!res) return;
